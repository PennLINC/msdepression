...
### Post: Yeo7 network mask resampled to template space, file <streamline_volume_within_${network}_network>, containing each fascicle and its corresponding % within yeo network
...
for network in "${yeo_7_net_list[@]}"; do
	...
	rm -f $outfile
	touch $outfile
	echo "fascicle,num_voxels_total,non_zero_voxels_in_${network}_map,prop_in_mask" >> $outfile
	...
	for fiber_bundle_type in ${fiber_bundle_type_list}; do
	    ...
	    for fascicle in ${fascicle_volumes}; do
		    ...
		    if [ -f "${fascicle_direc}/${fascicle_type}/${fascicle_prefix}${network}_mask.nii.gz" ]; then
