...
#set data_empi_acc_f_phq to data3
data_empi_acc_f_phq <- data3#subset(data, select = c("EMPI", "ACCESSION_NUM", "PHQ.2", "PHQ.9"))

#number of unique in melissa's
m_access_n <- length(unique(melissa_xls$ACCESSION_NUM)) #n = 29183

#number of unique in my group
eb_access_n <- length(unique(data_empi_acc_f_phq$ACCESSION_NUM)) #n = 21027

#number of overlap

melissa_peeps_in_eb_group <- sum(unique(melissa_xls$ACCESSION_NUM) %in% unique(data_empi_acc_f_phq$ACCESSION_NUM)) #n = 15137

eb_peeps_in_melissa_group <- sum(unique(data_empi_acc_f_phq$ACCESSION_NUM) %in% unique(melissa_xls$ACCESSION_NUM)) #n=15137
...
num_phq2_and_dep <- length(which(!is.na(data_empi_acc_f_phq$ICD9) & !is.na(data_empi_acc_f_phq$PHQ.2))) #n = 4855
print(paste0("N with ICD 9/10 codes for depression and phq2 = ", num_phq2_and_dep))

num_phq9_and_dep <- length(which(!is.na(data_empi_acc_f_phq$ICD9) & !is.na(data_empi_acc_f_phq$PHQ.9))) #n=669
print(paste0("N with ICD 9/10 codes for depression and phq9 = ", num_phq9_and_dep))

#fcode histos
hist(data_empi_acc_f_phq$PHQ.2[which(!is.na(data_empi_acc_f_phq$ICD9) & !is.na(data_empi_acc_f_phq$PHQ.2))], 
     main = paste0("PHQ2 and Dep Histo : n = ", length(which(!is.na(data_empi_acc_f_phq$ICD9) & !is.na(data_empi_acc_f_phq$PHQ.2)))), xlab = paste0("PHQ2"))

hist(data_empi_acc_f_phq$PHQ.9[which(!is.na(data_empi_acc_f_phq$ICD9) & !is.na(data_empi_acc_f_phq$PHQ.9))], 
     main = paste0("PHQ9 and Dep Histo : n = ", length(which(!is.na(data_empi_acc_f_phq$ICD9) & !is.na(data_empi_acc_f_phq$PHQ.9)))), xlab = paste0("PHQ9"))
...
#remove people who do not have phq2 or 9
data_e_with_phq2 <- data_e[which(!is.na(data_e$PHQ.2)),] #4864
data_e_with_phq9 <- data_e[which(!is.na(data_e$PHQ.9)),] #376
