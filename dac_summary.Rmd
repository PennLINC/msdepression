...
eb_icd_and_phq2 <- data_e$ACCESSION_NUM[which(!is.na(data_e[dep_df,]$PHQ.2))] #n = 741
eb_icd_and_phq9 <- data_e$ACCESSION_NUM[which(!is.na(data_e[dep_df,]$PHQ.9))] #n = 106

#overlap of groups
n_melissa <- sum(unique(melissa_xls$ACCESSION_NUM) %in% unique(data_e$ACCESSION_NUM))

#unique empis
n_melissa_unique_empi <- length(unique(melissa_xls$EMPI[(unique(melissa_xls$ACCESSION_NUM) %in% unique(data_e$ACCESSION_NUM))]))
...
print(paste0("n people in Melissa and EB overlap group, unique empi: ", n_melissa_unique_empi, " out of ", length(unique(data_e$ACCESSION_NUM))))
