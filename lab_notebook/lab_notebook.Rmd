...
- Getting rid of some of the within network analyses in the replication. Just going to do interactions because they look great.

``` {r teriles_and_quintiles}
...
also getting rid of some commented out code for saving things

``` {r commented_out_stuff_from_quintile_tertile_section}
...
Getting rid of effect size regression with cohen d (b/c fascicles aren't normally distributed)
``` {r effect_size_regression}
#  text_for_eval <- paste0("cohensD(dep_and_healthy_groups_for_ICD_analysis$", fascicle, "[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 2], dep_and_healthy_groups_for_ICD_analysis$", fascicle, "[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 1])")
```

#### 20220908

[x] got replication off to Elizabeth on 9/7
[x] Making figures
  [x] For brains, made red to yellow map for volume of overlap w/depression network (scaled w AF, not cc because it is too big)
    [x] brain results file: /Users/eballer/BBL/msdepression/volume_of_overlap_w_dep_net_n77
    [x] color file: /Users/eballer/BBL/msdepression/templates/dti/colors/volume_of_overlap_w_dep_net_n77_ry_scaling_cc.txt
  [x] made pink gradient map for the vol/effect size map
    [x] brain results file: /Users/eballer/BBL/msdepression/fascicle_and_wilcox_effect_size_for_volxdx_analysis_n77
    [x] color file: /Users/eballer/BBL/msdepression/templates/dti/colors/fascicle_and_wilcox_effect_size_for_volxdx_analysis_n77_pg.txt
  [x] made binary map of volume
    [x] brain results file: /Users/eballer/BBL/msdepression/volume_of_overlap_w_dep_net_n77
    [x] color file: /Users/eballer/BBL/msdepression/templates/dti/colors/volume_of_overlap_w_dep_net_n77_pinkblue.txt

[x] make figure of depression network from bold
  [x] resampled the harvard map to mni_icbm_2009b template: 
  [x] multiplied the resampled image with the mask
  [x] displayed in fsleyes and looks amazing
    underlay: mni_icbm152_t1_tal_nlin_asym_09a_mask.nii (drop intensity a bit to get the background not to have that ring around it)
    overlay: DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked.nii (set to 3.09/3.09 for yellow, 3.09/5 for colored)

```{bash resampling}

 #resample to get in same space

 3dresample -master mni_icbm152_t1_tal_nlin_asym_09a.nii -input DepressionCircuit_t.nii -prefix DepressionCircuit_t_resampled_to_mni_icbm152_asym09a.nii
 
 #mask
  3dcalc -a DepressionCircuit_t_resampled_to_mni_icbm152_asym09a.nii -b mni_icbm152_t1_tal_nlin_asym_09a_mask.nii -expr 'a*b' -prefix DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked.nii
  
# display
 fsleyes
 #underlay: mni_icbm152_t1_tal_nlin_asym_09a_mask.nii
#overlay: DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked.nii
```
  
  
```{r make_red_to_yello_RGB_color_scheme_accounting_for_cc} 
...
#### 20220913
1. [o] Flow chart (text is small, do I need to make bigger?)
  efile a.[x] demographics full
  efile b.[x] (promis scores)
  efile c. [] pipeline is e -file (can only have max 4 pt figure), will need to work with Maddie for this.
2. Making depression network
  [x]: Cortical rendering of depression network
  [x]: DSI fascicle of volume of overlap w/network (how to scale for visualizations) (new hampshire forest in the fall colors)
3.  In vs outside dep net
    a. inside vs outside dep net violin
    b. pink and blue fascicle picture part B
4. Diagnosis analysis
    a. no diff by lesion volume (violin)
    b. but difference in fascicle volume (violin)
        - just in/outside of network (put stuff about within/outside network for each diagnostic group in text)
5. Dep Network/dx interactions
     a. interaction graph
     b. scatter (effect size by overlap)
     c. Fascicles colored by effect size (edited)    
...
[] Elizabeth is currently replicating
  - found a bug in the naming order of one of the files. We have changed it, and the subsequent results are the same, but a little better
  - The mask stays the same (dep vs non), so no need to regenerate those pictures
  - Coloring for new hamphire colors and pink effect size are slightly different
    - [x] redid the new hampshire color plot
    - [x] redid pink plot
  - Will need to remake the main effect of dx and network violins with new data
    [x] main effect of network
    [x] main effect of diagnosis
  - Remake scatter and interaction plots
    [x] scatter
    [x] interaction
[x] review some sample figures
[x] talk about science for LRP
  - spoke with Mark, feasibility important
  - In line with that, do I convert aim 1 to 2 aims (have sex differences be one)?
  - Or (my preference), basically keep everything the same but state that aim 2 will be the beginning of a collection of data (50 people per year), rather than 250. 
    - would be much eaasier to do this rather than rewrite that whole chunk.
    - Also, mentorship team from K, but without mike fox and tom. Substitute sex differences for EMR training?
  - I have also included that I applied for a K already, and what my plans are if I don't get it
  - 
  
#### 20220915
  [] cleaning up scatter with Elizabeth - incremented the volume by 1, and log transformed for a much prettier graph!
  [x] Making the binary brain map on an inflated brain in workbench
   - first, binarized it 
```{bash}
3dcalc -a DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked.nii -expr 'ispositive(a-3.09)' -prefix DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked_binarized_3_09.nii
```
   - tried to use command line tools for workbench, couldn't do it, but val passed on a script that was just amazing, and I adapted it for my stuff
```{r}
...
- I then had to take this into workbench, and futz with the colors

  - open application: wb_view
  - open file: /Users/eballer/BBL/msdepression/templates/harvard_depression/testvol2surf.dscalar.nii
  - open file: /Users/eballer/Software/workbench/workbench_files/HCP_S1200_GroupAvg_v1/S1200.L.midthickness_MSMAll.32k_fs_LR.surf.gii
  - open file: /Users/eballer/Software/workbench/workbench_files/HCP_S1200_GroupAvg_v1/S1200.R.midthickness_MSMAll.32k_fs_LR.surf.gii
  - click montage
  - under settings, click the wrench
  - Threshold: Off
  - Palette
    - Pick the palette HSB8_clrmid
    - Invert off
    - click interpolate colors
    - click fixed
    - setting 40/0/0/0
  - click negative and positive
  - Click bars under histogram
  - change bars color and envelope to fuchsia (don't know if this does anything)
  - Color bar and palette settings: apply to all maps

- Save, screenshot, bring to illustrator and have at it

- Also, will do log scatter, looks so much better. Have to increment by 1 in order to make it pretty though, otherwise all the 0 values will screw you over

#### 20220921

[x] replicated!
[x] Updated the stats in my markdown and changed the name to reflect that it is post replication
[x] big changes - correcting the volume multiplication error
[x] Also changed the effect size by volume plot to be scaled logarithmicly
  [x] Now p value of this regression is 0.0187 (Adjusted R2 0.06).
  [x] Elizabeth thinks we can drop the Boot. Ted??
[x] colors for depressed vs nondepressed plots
[x] figure alternatives (move interaction to figure 4, and have fig 5 just brain + scatter)

#### 20220922

[x] no cerebellum -> should I find a way to do a flat representation? (NO)

Need to install python and pennlinc toolkit - did not work. It is broken. Don't use it

#### 20220930 or next week
[x] add post hoc */bars to figure 4
  - things that are significant - depressed vs nondepressed network for each (big sig), dep net between dep & nondep (p=0.04); non dep network nonsig (p = 0.051)
[x] add legend
[x] call it a win!
[x] add statistics to other graphs (p/effect size)
[x] fix flow chart
  - why is it that when you combine a data frame where the EMPIs overlap by 905 people, do you end up with only 783 unique EMPIs?
  - perhaps the data_empi_ etc pulls people with scans that aren't on the same day as the ones that are mimosa'ed
  - So what that would mean is when the merge of subject and exam date happens, some things are dropped. it is possible that there are more subjects that overlap in EMPI, but that scans were pulled on different dates (suggesting that there are missing scans, or scans that aren't in the big spreadsheet but are in the mimosa files).
  - So i will merge data_empi with the mimosa qc, and check to see how many people were available in that overlap. And that will become my number I think.
[x] start writing science

#### 20230113
[x] Wilcoxon eff size not working, can report everything from the mixed effect model
[x] Made new code msdepression_n380_started_20230113_post_replication_just_simple_mm.Rmd
[x] removing wilcoxon code
[x] getting confidence intervals for mixed model with confint
[x] in reporting, beta (estimate), CI (from confint), p value

#### 20230510
[x] going back to do some Yeo stuff. Made sure I could sum up each individual's Yeo network injury

#### 20230511
[x] make proportions of yeo injury by dividing by total volume within network
[x] compare disease across networks 
[x] compare disease within network across diagnoses

#### 20230517
[x] added sensitivity analyses where I dropped corpus callosum. Findings still great.
[x] need to update paper with this analysis.


#### 2023 0621

[x] Interested in whether lesions to fibers involved in serotonergic projections (primarily UF) are more associated with depression (or response to SSRIs, I imagine people who have completely obliterated it - bad, people who have some left - okay, people who have healthy - maybe not as much impact. Inverted u-shaped curve). No differences in UF

```{r}

#same results when using volume. No difference with UF; W = 16268, p-value = 0.3762
dep_vs_healthy_wilcoxon_lesion_volume_results <-  wilcox.test(dep_and_healthy_groups_for_ICD_analysis$UF_L_prop[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 1], dep_and_healthy_groups_for_ICD_analysis$UF_L_prop[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 2], paired = F)

#same results when using volume. No difference with UF; W = 16076, p-value = 0.2914
dep_vs_healthy_wilcoxon_lesion_volume_results <-  wilcox.test(dep_and_healthy_groups_for_ICD_analysis$UF_R_prop[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 1], dep_and_healthy_groups_for_ICD_analysis$UF_R_prop[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 2], paired = F)
```
[] Same with NE

[] What if you ranked fascicles based on their overlap with GM areas that have 5HT receptors
 - need a dasb mask essentially
 - 5ht1a mask
 - 5ht2a?
 - SERT (dasb)
 
[] Birth control study
  - disease differences in people on/off bcp?
  - longitudinal outcome models?
  
20230824
SOBP analyses
[] age x sex, age x dep 
[x] looking at indiv fascicles in dep network, figure out which ones independently separate dep vs non - theoretically do prop lost ~ dep diagnosis
   - only uncorrected findings
[] 

20230829
[] take each map from golia, project to surface, spin test for overlap with depression map from shan. Rank by overlap. Make white matter backbones. Test for lesion network enrichment in depression group. correlate effect sizes from spin correlation. Idea would be that maps that were more highly related to depression network are also more associated with pt vs control differences. And also, hypothesize that these areas are enriched for major depression neurotransmitters
[] also consider receptor density studies. 
[] if you find that fascicles that feed into areas with higher receptor densities, could be data for PET
[] also wonder whether medication selection using brain imaging data could be useful for depression

#### 20230927 analyses for BP revision

[x] Add sex into model
...
[x] Prevalence of depression in the PROMIS cohort
...
[x] use different threshold of 
  [x] 2.3
  [x] 2.6
```{bash}
cd /Users/eballer/BBL/msdepression/templates/harvard_depression
#we are going to threshold the DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked.nii at 2.3 and 2.6
3dcalc -a DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked.nii -expr 'ispositive(a-2.3)' -prefix DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked_binarized_2_3.nii
3dcalc -a DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked.nii -expr 'ispositive(a-2.6)' -prefix DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked_binarized_2_6.nii

#send to pmacs
scp DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked_binarized_2_* eballer@bblsub.pmacs.upenn.edu:/project/msdepression/templates/harvard_depression/.

#on pmacs

#change the scripts
more calc_vol_fascicles_within_depression_mask_3_09.sh  | perl -pe 's/3\.09/2\.3/' | perl -pe 's/3_09/2_3/' > calc_vol_fascicles_within_depression_mask_2_3.sh

more calc_vol_fascicles_within_depression_mask_3_09.sh  | perl -pe 's/3\.09/2\.6/' | perl -pe 's/3_09/2_6/' > calc_vol_fascicles_within_depression_mask_2_6.sh

#go in to each script individually and update the mask to be DepressionCircuit_t_resampled_to_mni_icbm152_asym09a_masked_binarized_2_3.nii and 2_6 individually

#run the scripts
sh calc_vol_fascicles_within_depression_mask_2_3.sh
sh calc_vol_fascicles_within_depression_mask_2_6.sh

#send output files back to home; from local, needed to do without wildcard because zsh was giving me a hard time
scp eballer@bblsub.pmacs.upenn.edu:/project/msdepression/results/streamline_volume_within_dep_network_2_6.csv /Users/eballer/BBL/msdepression/results/.
scp eballer@bblsub.pmacs.upenn.edu:/project/msdepression/results/streamline_volume_within_dep_network_2_3.csv /Users/eballer/BBL/msdepression/results/.

```

[] a couple of other analyses
- people on Interferon B (11 total, 5 in nondepressed, 6 in depressed; solumedrol 2/2)
```{r}
table(dep_and_healthy_groups_for_ICD_analysis$depGroupVar[grep(pattern = "INTERF", x = dep_and_healthy_groups_for_ICD_analysis$Medication)])
```
- people on steroids (prednisone 1 in nondep, 1 in dep)
- 
```{r}
```
- people on ocrelizumab (17 in healthy group, 14 in depressed group)
```{r}
table(dep_and_healthy_groups_for_ICD_analysis$depGroupVar[grep(pattern = "OCRELIZUMAB", x = dep_and_healthy_groups_for_ICD_analysis$Medication)])
```

Could also do sensitivity analysis and remove the people on interferon Beta, prednisone, methylprednisolone
```{r}
dim(dep_and_healthy_groups_for_ICD_analysis %>% filter(!str_detect(Medication, 'INTERFERON|PREDNISON|METHYLPREDNISOLONE')))  #remove
