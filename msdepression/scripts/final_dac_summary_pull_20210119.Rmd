...
healthyish_phq9_subset <- phq9_subset[which(phq9_subset$PHQ.9 <= 4),] #n = 55
healthyish_unique_phq9_empis <- unique(healthyish_phq9_subset$EMPI) #n =9
...
subset_with_qc <- merge(subset, melissa_qi_data, by = c("EMPI", "EXAM_DATE"))
...
fascicle_age_lm <- lapply(fascicle_names, function(x) 
{
  lmerTest::lmer(substitute(i ~ PAT_AGE_AT_EXAM, list(i = as.name(x))), data = dep_and_healthy_groups_for_ICD_analysis)
})
names(fascicle_age_lm) <- fascicle_names
