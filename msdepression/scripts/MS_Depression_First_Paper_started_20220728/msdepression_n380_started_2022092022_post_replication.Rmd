...
#### Diagnostic Effect
    ...
    - Proportion Volume of whole brain fascicle disease burden: W=14996, p=0.04, dep = 0.39, nondep = 0.34, eff size 0.11 (small)
...
#### Network x Diagnosis Interaction
    ...
    - Main effect of Network: T = 19.2, p <2 * 10^-16 (Depression Network more disease than nondepression network)
...
#### Sensitivity Analyses
      ...
      2) Quintiles
        ...
        - Main effect of diagnosis (Depressed > Nondepressed people): T = 2.4, p = 0.048
...
disease_overall_shapiro <- shapiro.test(dep_and_healthy_groups_for_ICD_analysis$proportion_volume_lost_per_total_network_size_sum) #W=0.97, p = 4.1 * 10^-7, NON-NORMAL

disease_dep_net_shapiro <- shapiro.test(dep_and_healthy_groups_for_ICD_analysis$proportion_volume_lost_dep_net_by_network_size_sum) #W=0.97, p = 2.0 * 10^-7, NON-NORMAL

disease_nondep_net_shapiro <- shapiro.test(dep_and_healthy_groups_for_ICD_analysis$proportion_volume_lost_nondep_net_by_network_size_sum) #W=0.97, p = 2.6 * 10^-7, NON-NORMAL

disease_dep_dx_shapiro <- shapiro.test(dep_and_healthy_groups_for_ICD_analysis$proportion_volume_lost_per_total_network_size_sum[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 2]) #W=0.97, p = 0.0003 NON-NORMAL
...
### Within/outside depression network, V = 65897, p = 2.2*10^-16, mean dep net prop = 0.41, mean non dep = 0.31
...
#within nondepressed group, V=9661, p <2.2*10^-16
msdisease_within_versus_outside_depression_network_nondep_group_sums_scaled_by_size_wilcoxon <- wilcox.test(dep_and_healthy_groups_for_ICD_analysis$proportion_volume_lost_dep_net_by_network_size_sum[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 1], dep_and_healthy_groups_for_ICD_analysis$proportion_volume_lost_nondep_net_by_network_size_sum[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 1],paired = TRUE) 


#within depressed group, V= 25143, p<2.2*10^-16
msdisease_within_versus_outside_depression_network_dep_group_sums_scaled_by_size_wilcoxon <- wilcox.test(dep_and_healthy_groups_for_ICD_analysis$proportion_volume_lost_dep_net_by_network_size_sum[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 2], dep_and_healthy_groups_for_ICD_analysis$proportion_volume_lost_nondep_net_by_network_size_sum[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 2],paired = TRUE) 



### Diagnostic effects

#proportion volume lost over whole network = W=14996, p=0.04, mean dep = 0.39, mean nondep = 0.34
...
#proportion volume lost within depression network, W = 14969, p-value = 0.04; dep = 0.44, nondep = 0.38
...
#proportion volume lost outside depression network W = 15134, p-value 0.051; mean dep = 0.32, mean nondep = 0.29
...
#effect size within vs outside dep network, eff size = 0.78 (large)
...
#effect size dep vs nondep network within nondepressed, eff size = 0.75 (large)
msdisease_within_versus_outside_depression_network_nondep_group_sums_scaled_by_size_wilcoxon_eff_size <- rstatix::wilcox_effsize(df_empi_dx_network_proportions[(df_empi_dx_network_proportions$Diagnosis == "Nondepressed"),], formula = proportion_affected ~ Network, paired = T, conf.level = 0.95, ci.type = "perc", nboot = 1000)

#effect size dep vs dep network within depressed, eff size = 0.80, large
msdisease_within_versus_outside_depression_network_nondep_group_sums_scaled_by_size_wilcoxon_eff_size <- rstatix::wilcox_effsize(df_empi_dx_network_proportions[(df_empi_dx_network_proportions$Diagnosis == "Depressed"),], formula = proportion_affected ~ Network, paired = T, conf.level = 0.95, ci.type = "perc", nboot = 1000)

#diagnosis: Dep > nondep T = 2.4, p = 0.015; Network dep>nondep = T=19.2, p = 2*10^-16; intx = T = 2.3, p = 0.020
...
Diagnosis_labels <- c("Depressed", "Nondepressed")
...
bar_dep_vs_nondep_scaled_by_network_size <- ggplot(df_proportion_network_volume_and_diagnosis_summary, aes(x=depGroupVar, y=mean, fill=depGroupVar)) +
  ...
  scale_x_discrete(labels=c("Non-depressed" = "Nondepressed", "Depressed" = "Depressed")) + 
  ggtitle(paste0("Proportion Fascicle Volume Affected by Diagnosis")) +
  ylab("Proportion (sum total volume lost/Net Vol) ") +
  ylim(0,0.5) + 
  theme(plot.title = element_text(size=16, hjust = 0.55),
        axis.text = element_text(size = 16, colour = "black"), axis.title.x = element_blank(), 
        axis.title.y = element_text(size=16),
        axis.line = element_line(size = 1.5, colour = "black", linetype=1),
        legend.position = "none",
        aspect.ratio=7/5,
...
#save as pdf
pdf(file=paste0(homedir, "/results/Figures_for_msdepression_paper/diagnosis_by_network_interaction_line_graph_range0to05_point.pdf"))
  interaction_line
dev.off()
...
#geom_point 0.2-0.5
interaction_ggplot_point_02_05 <- ggplot(data = df_interaction_fit_upper_lower, mapping = aes(x = Diagnosis, y = model_data$predicted, color = Network, group = interaction(Diagnosis, Network))) + 
    geom_errorbar(aes(ymin=model_data$conf.low,ymax=model_data$conf.high), position=position_dodge(width=0.5), width = 0.2,size = 1.5, color='black') +
    geom_point(position = position_dodge(width=0.5), size = 5) + 
    scale_color_manual(values=Network_colors, labels = c("Depression Network", "Nondepression Network")) + 
    ylim(0.2,0.5) + 
    ...
    theme(axis.text.x = element_text(size=16, color="black"),
          ...
          legend.position = "top") #+ 
 #   ggtitle("Diagnosis x Depression Network Disease")
print(interaction_ggplot_point_02_05)

#save as pdf
pdf(file=paste0(homedir, "/results/Figures_for_msdepression_paper/diagnosis_by_network_interaction_plot_range0_2to0_5_point.pdf"))
  interaction_ggplot_point_02_05
dev.off()
...
#save as pdf
pdf(file=paste0(homedir, "/results/Figures_for_msdepression_paper/diagnosis_by_network_interaction_plot_range0to05_bar.pdf"))
...
pdf(file=paste0(homedir, "/results/Figures_for_msdepression_paper/diagnosis_intx_line_and_intx_bargraph_range0to05.pdf"))
...
#V=67727, p=2.2*10^16, top tertile prop vol affected = 0.42, bottom 2 tertiles = 0.28
...
#p=2.2*10^-16, top quintile prop vol affected = 0.43, bottom 4 quintiles = 0.30
