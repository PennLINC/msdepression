...
Total n promis scores: 1694 
Total n unique individuals w/promis scores: 501

Different scores have different # of overall scans
                        Total                      General_Health                  Quality_of_Life                  Physical_Health 
                            1694                             1693                             1503                             1503 
...
W/good mimosa:
Total n promis scores : 398
Total n unique individuals w/promis scores: 116
Total n unique indivs in nondepressed group = 36
Total n unique individuals in depressed group = 46
...
Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
    1.0   376.8   763.0   873.3  1229.8  2487.0 
    Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
0.002747 1.035027 2.096154 2.399132 3.378434 6.832418 
...
Promis results:
Interestingly, across all 11 scores, with the exception of fatigue, Depressed patients have lower scores (lower = worse functioning), than non-depressed patients. controls. They all fdr correct. What I love about this is fatigue not being different. If fatigue was worse in depressed patients, one could argue that quality of life, health, emotional problems, social activities, etc are all worse because of underlying fatigue. Similarly, as fatigue is a component of the PHQ9 and depression screening, this could be confounding. That they were different in a lot of measures but not fatigue is informative.

Quality_of_Life	0.015199144			
Physical_Health	0.012579264			
Mental_Health_and_Mood	0.012579264			
Social_Activities_Satisfaction	0.001063449			
Carrying_Out_Social_Activities	0.011375799			
Carrying_Out_Physical_Activities	0.031616473			
Emotional_Problems	0.011270683			
Fatique_Average	0.231497853			
PostOp.PROMIS.Physical.Score	0.041172446			
PostOp.PROMIS.Mental.Score	0.001253404	
...
sum(fascicle_volumes_dep_network$num_voxels_total[which(fascicle_volumes_dep_network$fascicle %in% dep_network_names_top_quartile_no_cranial_nerves_by_vol)])
...
sum(fascicle_volumes_dep_network$num_voxels_total[which(fascicle_volumes_dep_network$fascicle %in% dep_network_names_bottom_3_quartiles_no_cranial_nerves_by_vol)])
...
dep_net_group <- c(rep("Dep_net", times = 380), rep("Non_dep_net",times = 380))
...
violin_within_vs_outside_network_scaled_by_network_size <- ggplot(newdf_no_cc, aes(x=dep_net_group, y=net_values, fill=dep_net_group)) +
  ...
  ylab("Proportion (tot volume lost/Net Vol) ") +
  ylim(0,0.1) + 
  theme(plot.title = element_text(size=16, hjust = 0.55),
        axis.text = element_text(size = 14, colour = "black"), axis.title.x = element_blank(), 
        axis.title.y = element_text(size=16),
