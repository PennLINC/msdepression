...
#wilcox test because non normally distributed; sums instead of means, scaled by network size p < 2.2*10^-16
...
df_empi_dx_network_proportions <- dep_and_healthy_groups_for_ICD_analysis %>%
  ...
  mutate(diagnosis = as.factor(depGroupVar)) %>%
...
diagnosis_by_network_interaction_lmer <- lmer(proportion_affected ~ network*diagnosis + (1|subject), data = df_empi_dx_network_proportions)
...
diagnosis_by_network_interaction_lmer_plot <- plot_model(diagnosis_by_network_interaction_lmer, type = "int")
