...
```{r comparing_depression_map_overlap_with_dx_diff_no_cranial_nerves_volume_no_CC}
...
```{r comparing_depression_map_overlap_with_dx_diff_no_cranial_nerves_vol_effect_size_no_CC}
# the purpose of this chunk is to plot the relationship of the degree of overlap of a depression network with the diagnostic difference between groups
...
df_fascicle_dep_mask_volume_overlap_d_value_no_cc <- df_fascicle_dep_mask_volume_overlap_d_value %>%
  filter(fascicle != "CC")

#make copy to sort
df_fascicle_dep_mask_overlap_d_value_sorted_by_vol_overlap_depression_mask <- df_fascicle_dep_mask_volume_overlap_d_value_no_cc 
...
plot_fascicle_by_d_sorted_by_dep_overlap <-ggplot(data = df_fascicle_dep_mask_overlap_d_value_sorted_by_vol_overlap_depression_mask, aes(x = fascicle, y=dep_vs_healthy_d), main = "Dep vs Healthy T value, Fascicles ordered by overlap with dep network, no CC") + 
  geom_bar(stat="identity") +
  #ylim(-0.1,0.5) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
print(plot_fascicle_by_d_sorted_by_dep_overlap)
...
#statistic, p=001
dep_overlap_x_dep_vs_healthy_vol_d <- lm(dep_vs_healthy_d ~ volume_in_dep_mask, data=df_fascicle_dep_mask_volume_overlap_d_value_no_cc )
print(summary(dep_overlap_x_dep_vs_healthy_d))

scatter_d_val_by_dep_net_overlap <- ggplot(data=df_fascicle_dep_mask_volume_overlap_d_value_no_cc , aes(x=volume_in_dep_mask, y=dep_vs_healthy_d)) +
  geom_point(size=1) + 
  geom_smooth(method=lm)+
  xlab("Volume of Fascicle in Depression Network") + 
  ylab("Effect of diagnosis calculated in Volumes (D)") + 
  ggtitle(paste0("Fascicle/depression mask Volume overlap by dx Eff Size, No CC p = ", round(summary(dep_overlap_x_dep_vs_healthy_vol_d )$coefficients[2,4],3))) + 
  theme(plot.title=element_text(hjust=0.5))
print(scatter_d_val_by_dep_net_overlap)
...
# 95% confidence intervals for the correlation
set.seed(1)
results_vol <- boot(data=df_fascicle_dep_mask_volume_overlap_d_value_no_cc, statistic=tval,
   R=10000, formula=dep_vs_healthy_d~volume_in_dep_mask)
print(results_vol)
plot(results_vol)
boot_results_vol.ci <- boot.ci(results_vol, conf=0.95, type="perc", index=1)
