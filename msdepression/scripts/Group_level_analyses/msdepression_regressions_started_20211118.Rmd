...
data_processed_as_of_20220209 <- read.csv(paste0(homedir, "/data/msdepression/data/erica_dac_pull/accession_numbers_tim_updtes_20220209.csv"), header = TRUE, stringsAsFactors = FALSE, sep = ",") #n=2738 new subjects added, 1124 were previously in Melissa's group, 2732 were already in our sample. Total n = 3862. Total accession # = 16,853
...
```{r depression_unique_all_fascicles_together_last_scan}
...
dep_vs_healthy <- lmerTest::lmer(total_fascicle_prop_lost ~ depGroupVar + (1 | EMPI) + PAT_AGE_AT_EXAM, data = dep_and_healthy_groups_for_ICD_analysis) #trend, p = 0.15

#dep_easy <- lm(total_fascicle_prop_lost ~ depGroupVar + PAT_AGE_AT_EXAM, data = dep_and_healthy_groups_for_ICD_analysis)

dep_vs_healthy_net <- lmerTest::lmer(total_fascicle_prop_lost_indepnet ~ depGroupVar + (1 | EMPI) + PAT_AGE_AT_EXAM, data = dep_and_healthy_groups_for_ICD_analysis) #trend, p = 0.14
...
dep_vs_healthy_net_10_percent <- lmerTest::lmer(total_fascicle_prop_lost_indepnet_10_percent ~ depGroupVar + (1 | EMPI) + PAT_AGE_AT_EXAM, data = dep_and_healthy_groups_for_ICD_analysis) #p = 0.23
...
#anova
fascicle_agedep_anova <- lapply(fascicle_agedep_lm, anova) #p unc= C_PH_L, p = 0.03, PAT_R p=0.04, slf1_R p = 0.09, VOF_R p = 0.03
...
### Chi sq - compare # fdr corrected values in dep network to outside
table_for_chisq <-data.frame(rbind(c(5,35), c(30,17)), row.names = c("nondep_net", "dep_net"))
names(table_for_chisq) <- c("fdr05", "NS")
dep_network_age_effects_chi <- chisq.test(table_for_chisq) #p = 3.374 x 10^-6
...
total_volume_lost_dep_vs_healthy_unique <- t.test(dep_and_healthy_groups_for_ICD_analysis$total_fascicle_prop_lost[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == 1], dep_and_healthy_groups_for_ICD_analysis$total_fascicle_prop_lost[dep_and_healthy_groups_for_ICD_analysis$depGroupVar == -1]) #mean dep = 28%, mean non_dep = 25%, p = 0.12

volume_lost_dep_vs_nondep_network_unique <- t.test(dep_and_healthy_groups_for_ICD_analysis$total_fascicle_prop_lost_indepnet, dep_and_healthy_groups_for_ICD_analysis$total_fascicle_prop_lost_nondep_network) #mean dep_net = 35%, mean non_dep_net =  17%, p 2.2 x 10^-16
...
dep_and_healthy_groups_for_ICD_analysis$total_fascicle_prop_lost_nondep_network) #mean dep net = 31%, mean non_dep_net = 17%, p 6.6 * 10^-15
...
loss_in_nondep_network <- lm(total_fascicle_prop_lost_nondep_network~depGroupVar, data = depression_network_and_dx_subgroup) #non sig, trend 0.09, mean dep = 19% loss, healthy 17% loss
```
